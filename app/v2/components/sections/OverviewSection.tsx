import type { HomePageData, WorkPageData } from "@/types/cms";
...
export default function OverviewSection({ data, expertise }: OverviewSectionProps) {
  ...
  return (
    <>
          ...
          <ScrollReveal direction="up" delay={300} className="bg-[rgba(30,41,59,0.7)] backdrop-blur-[12px] border border-[rgba(255,255,255,0.1)] shadow-[inset_0_0.5px_0_0_rgba(255,255,255,0.1)] p-8 rounded-2xl flex flex-col justify-between">
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {coreTools?.map((tool) => (
                <span
                  key={tool.label}
                  className="inline-flex items-center gap-1.5 px-3 py-1 rounded text-[14px]"
                  style={{
                    background: "#222a3d",
                ...
                >
                  <TechIcon label={tool.label} className="text-[15px] shrink-0" />
                  {tool.label}
                </span>
              ))}
            </div>
      ...
      {/* ── Core Expertise ────────────────────────────────────────────────────── */}
      <section className="px-6 pb-12 md:pb-28 max-w-v2-container mx-auto">
        <CoreExpertise expertise={expertise} />
      </section>
